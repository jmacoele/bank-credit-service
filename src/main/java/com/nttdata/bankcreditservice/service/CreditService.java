...
public class CreditService implements ICreditService  {
	...
	private void validateCredit(Credit credit) throws Exception {
		...
		if (credit.getCreditType().equals(CreditTypeEnum.PERSONAL)) {
			...
			creditRepository
					...
					.then(Mono.error(new Exception("Credito personal solo se permite un solo crédito por persona.")));	
		}
	...
	@Override
	public Mono<Credit> save(String id, Credit credit) throws Exception {
		validateCredit(credit);
		return this
				.creditRepository
				.save(credit);
